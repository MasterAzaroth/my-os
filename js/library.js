...
function setupPTR(wrapperId, scrollId, spinnerId, refreshFn) {
  ...
  scroll.addEventListener('touchmove', e => {
    ...
    if(dy <= 0) return;
    // Rubber band: ease off toward max
    const clamped = Math.min(dy, MAX_DRAG);
    const eased = clamped * (1 - clamped / (MAX_DRAG * 2.5));
    const progress = Math.min(dy / TRIGGER_PX, 1);
    // Slide content down — spinner at top:-44px comes into view naturally
  ...
  scroll.addEventListener('touchend', () => {
    ...
    if(triggered && !refreshing) {
      // Snap both content and indicator back together, keep spinner visible
      scroll.style.transition = SNAP;
      scroll.style.transform = 'translateY(44px)';
      indicator.style.transition = SNAP;
      indicator.style.transform = 'translateY(44px)';
      ...
      // Didn't trigger — snap both back together
      scroll.style.transition = SNAP;
      scroll.style.transform = 'translateY(0)';
      indicator.style.transition = SNAP;
      indicator.style.transform = 'translateY(0)';
      spinner.style.transition = 'opacity 0.15s ease';
      spinner.style.opacity = '0';
